...
const About = () => {
  ...
  const educationData = [
    { wrapperClass: 'card-item-1', title: 'Bhaktavatsalam Vidyashram', specialization: 'General Stream', marks: '66.8%', year: '2015', smallInfo: ['10'], styleOverride: { position: 'relative', top: '-4vh' } },
    { wrapperClass: 'card-item-2', title: 'Bhaktavatsalam Vidyashram', specialization: 'Computer Science', marks: '73%', year: '2017', smallInfo: ['12'], styleOverride: { position: 'relative', top: '4vh' } },
    { wrapperClass: 'card-item-3', title: 'S.A. Engineering College', specialization: 'Computer Science Engineering', marks: '7.9 CGPA', year: '2021', smallInfo: ['CSE'], styleOverride: { position: 'relative', top: '-4vh' } },
    { wrapperClass: 'card-item-4', title: 'Current Status', specialization: 'Learning New Tech & Improving Daily', marks: '∞', year: '2025', smallInfo: ['Jobless'], styleOverride: { position: 'relative', top: '4vh' } },
  ];
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // clean existing triggers (hot reload)
      ...
      // Panel 2 per-letter animation (bump/overshoot)
      if (panel2TitleRef.current && panel2Ref.current) {
        const chars = panel2TitleRef.current.querySelectorAll('.char');
        if (chars.length) {
          const panelWidth = Math.max(panel2Ref.current.offsetWidth, window.innerWidth);
          const startXBase = Math.round(panelWidth * 0.45) + 150;
      ...
      cardConfig.forEach(({ sel, from }) => {
        ...
        ScrollTrigger.create({
          ...
          onEnter: () => gsap.to(el, { opacity: 1, y: 0, scale: 1, duration: 0.8, ease: 'back.out(1.1)' }),
          onLeave: () => gsap.to(el, { opacity: 0, y: from, scale: 0.95, duration: 0.5, ease: 'power2.inOut' }),
          onEnterBack: () => gsap.to(el, { opacity: 1, y: 0, scale: 1, duration: 0.8, ease: 'back.out(1.1)' }),
          onLeaveBack: () => gsap.to(el, { opacity: 0, y: from, scale: 0.95, duration: 0.5, ease: 'power2.inOut' }),
        });
      });
        ...
        <div className="flex min-w-[300vw] w-fit h-full relative z-[3]">
          ...
          <div ref={panel1Ref} className="min-w-[100vw] w-full h-full flex">
            ...
            <div className="w-3/4 flex flex-col">
              <div className="h-1/2 rounded-3xl p-6 m-4 bg-gray-800 overflow-hidden">
                <h1 ref={headingRef} className="text-6xl font-bold mb-4 plastic-font text-white">About me</h1>
              ...
              <div className="h-1/2 flex gap-4 px-4 pb-4">
                <div className="w-1/2 rounded-3xl p-2 bg-gray-800"><Scene /></div>
                <div className="w-1/2 rounded-3xl bg-gray-800"><SkillGrid /></div>
              </div>
            </div>
          </div>
          ...
          <div ref={panel2Ref} className="min-w-[100vw] w-full h-full relative bg-black overflow-hidden">
            <div className="w-full h-full relative z-40 flex flex-col items-center">
              <h2 ref={panel2TitleRef} className="text-center text-white text-7xl font-bold mt-8 mb-6 soda-shake-font" aria-label={titleText}>
                {titleText.split('').map((ch, i) => (
                  ch === ' '
                    ? <span key={`sp-${i}`} className="char space inline-block" style={{ width: '0.45em' }}>&nbsp;</span>
                    : <span key={`${ch}-${i}`} className="char inline-block">{ch}</span>
                ))}
              </h2>

              <div className="flex-1 flex items-center justify-center gap-[4vw]">
                {educationData.map((edu, idx) => <Card3D key={idx} {...edu} />)}
              </div>
            </div>
          </div>

          {/* Panel 3 */}
          <div ref={panel3Ref} className="min-w-[100vw] w-full h-full flex items-center justify-center bg-gray-700" />
        </div>
      </div>
    </section>
  );
